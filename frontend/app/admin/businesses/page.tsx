...
export default function AdminBusinessesPage() {
  const [businesses, setBusinesses] = useState<admin.AdminBusiness[]>([]);
  ...
  const fetchBusinesses = async () => {
    try {
      ...
      const response = await client.admin.listBusinesses({
  ...
  const filteredBusinesses = businesses.filter(biz => 
    ...
    biz.owner_username?.toLowerCase().includes(searchQuery.toLowerCase()) ||
    biz.owner_full_name?.toLowerCase().includes(searchQuery.toLowerCase())
  );
      ...
      >
        <div>
          ...
          <p className="text-gray-500 font-medium">
            Sistemdeki toplam {totalCount} işletmeyi yönetin.
          </p>
        </div>
        ...
        <div className="relative group">
          ...
          <input
            ...
            className="w-full md:w-80 bg-white border border-stone-200/60 pl-12 pr-4 py-3 rounded-2xl text-sm font-medium focus:outline-none focus:ring-4 focus:ring-violet-500/5 focus:border-violet-500 transition-all shadow-sm"
          />
        </div>
      </motion.header>
      ...
      <div className="space-y-6">
        {loading ? (
          ...
          <div className="text-center py-20 bg-white/40 rounded-[2.5rem] border border-dashed border-stone-200">
            ...
            <h3 className="text-stone-900 font-bold mb-1">İşletme Bulunamadı</h3>
            <p className="text-stone-500 text-sm font-medium">Arama kriterlerinize uygun işletme bulunmuyor.</p>
          </div>
        )}
      </div>
    </main>
