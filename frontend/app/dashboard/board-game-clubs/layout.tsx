...
import { BoardGameClubsProvider, useBoardGameClubs } from "./context";
import { getAppRootUrl } from "@/lib/apps";

function Sidebar() {
  const { id, business, club } = useBoardGameClubs();
    ...
    <aside className="w-80 h-full bg-white border-r border-stone-200/80 flex flex-col shrink-0 z-20 shadow-sm">
      <div className="p-6 border-b border-stone-100 space-y-5">
        ...
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 rounded-2xl bg-gradient-to-tr from-[#D4A830] to-[#E5B941] flex items-center justify-center text-white shadow-md shadow-[#D4A830]/10">
            <GameController size={24} weight="fill" />
          </div>
          <div className="min-w-0">
            <h1 className="font-black text-base text-stone-900 leading-tight truncate">Board Game Clubs</h1>
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-8">
        <div className="space-y-4">
          ...
          <div className="space-y-1.5">
            <button
              onClick={() => router.push(`/dashboard/board-game-clubs?id=${id}`)}
              className="w-full flex items-center gap-3 p-3 rounded-2xl text-left transition-all bg-stone-900 text-white shadow-lg shadow-stone-900/10 font-black"
            >
...
export default function BoardGameClubsLayout({ children }: { children: React.ReactNode }) {
  return (
    <BoardGameClubsProvider>
      <div className="flex h-screen w-screen overflow-hidden bg-[#FDFBF9] text-stone-900 font-sans relative">
        <Toaster position="top-center" />

        {/* Background decorations */}
        <div className="absolute inset-0 pointer-events-none z-0 overflow-hidden">
          <div className="absolute top-[-10%] left-[-15%] w-[60vw] h-[60vw] rounded-full bg-[#D4A830]/5 blur-[120px]" />
          <div className="absolute bottom-[-10%] right-[-15%] w-[60vw] h-[60vw] rounded-full bg-[#E5B941]/5 blur-[120px]" />
        </div>
