...
export default function BulkPage() {
  ...
  const handleDeleteAll = async () => {
    if (!confirm("Tüm menü verilerini (kategoriler ve ürünler) silmek istediğinize emin misiniz? Bu işlem geri alınamaz.")) {
      return;
    }

    try {
      setDeleting(true);
      toast.loading("Menü temizleniyor...", { id: "delete-menu" });
      
      const res = await client.digital_menu.deleteAllMenuData({ businessId: id });
      
      if (res.success) {
        toast.success("Menü tamamen temizlendi!", { id: "delete-menu" });
        await refreshData();
        refreshPreview();
      }
    } catch (err: any) {
      console.error(err);
      toast.error(`Hata: ${err.message}`, { id: "delete-menu" });
    } finally {
      setDeleting(false);
    }
  };
  ...
  const handleBulkImport = async () => {
    try {
      ...
      setImporting(true);
      toast.loading("Ürünler ve görseller işleniyor, lütfen bekleyin...", { id: "bulk-import" });

      // Use the new backend bulkImport endpoint which handles image downloads/uploads
      const items = Array.isArray(data) ? data : (data.items || []);
      
      const res = await client.digital_menu.bulkImport({
        businessId: id,
      ...
      if (res.success) {
        toast.success(`${res.count} ürün başarıyla eklendi!`, { id: "bulk-import" });
        setBulkJson("");
        await refreshData();
        refreshPreview();
      } else {
        throw new Error("İçe aktarma sırasında bir sorun oluştu.");
      }
    } catch (err: any) {
      console.error(err);
    ...
    "dietaryFlags": ["vegan"]
          ...
          <div className="flex justify-between items-center">
            <button
              ...
              className="flex items-center gap-2 px-6 py-4 text-red-500 hover:bg-red-50 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all disabled:opacity-50"
            >
            ...
            <button
              ...
              disabled={importing || !bulkJson.trim() || deleting}
