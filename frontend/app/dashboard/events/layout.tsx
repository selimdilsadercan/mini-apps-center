...
function Sidebar() {
  ...
  const { business, businessId } = useEvents();

  const menuItems = [
    {
      title: "Etkinlikler",
      icon: Calendar,
      path: `/dashboard/events?id=${businessId}`,
    ...
    <aside className="w-80 h-full bg-white border-r border-stone-200/80 flex flex-col shrink-0 z-20 shadow-sm">
      <div className="p-6 border-b border-stone-100 space-y-5">
        ...
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 rounded-2xl bg-gradient-to-tr from-[#00aeef] to-[#009bcf] flex items-center justify-center text-white shadow-md shadow-[#00aeef]/10">
            <Megaphone size={24} weight="fill" />
          </div>
          <div className="min-w-0">
            <h1 className="font-black text-base text-stone-900 leading-tight truncate">Events</h1>
...
function EventsLayoutContent({ children }: { children: React.ReactNode }) {
  const { business, loading } = useEvents();
  const router = useRouter();
  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden bg-[#FDFBF9] text-stone-900 font-sans relative">
      ...
      <div className="absolute inset-0 pointer-events-none z-0 overflow-hidden">
        <div className="absolute top-[-10%] left-[-15%] w-[60vw] h-[60vw] rounded-full bg-[#00aeef]/5 blur-[120px]" />
        <div className="absolute bottom-[-10%] right-[-15%] w-[60vw] h-[60vw] rounded-full bg-[#009bcf]/5 blur-[120px]" />
      </div>
    ...
    <Suspense
      fallback={
        <div className="flex h-screen bg-[#FDFBF9] items-center justify-center">
          <div className="w-12 h-12 border-4 border-[#00aeef]/10 border-t-[#00aeef] rounded-full animate-spin" />
        </div>
      }
    >
      <EventsProvider>
