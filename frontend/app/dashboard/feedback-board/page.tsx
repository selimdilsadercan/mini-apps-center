...
function FeedbackBoardAdminContent() {
  ...
  const publicUrl = typeof window !== "undefined" ? `${window.location.origin}/f?board=${businessId}` : "";
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-[#FDFBF9] px-6">
        <div className="max-w-md w-full bg-white rounded-2xl border border-zinc-200 shadow-sm p-8 text-center">
          ...
          <p className="text-sm text-zinc-500">Feedback yönetimi linkinde işletme ID&apos;si bulunamadı.</p>
        </div>
      </div>
    );
  }
            ...
            {feedbackListItems.map((item) =>
              item.type === "header" ? (
                <div
                  ...
                  className={`${activeFeedbacks.length > 0 ? "pt-5 mt-1 border-t border-zinc-200" : "pb-1"}`}
                >
                ...
                >
                  <div className="flex flex-col items-center justify-center w-14 min-h-[4rem] rounded-2xl border-2 border-zinc-300 bg-white shrink-0 px-2 py-2.5 gap-1">
                    ...
                    <span className="text-base font-black tabular-nums leading-none text-zinc-900">{item.feedback.vote_count}</span>
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-4 mb-2">
                      ...
                      <div className="relative">
                        <select
                          ...
                          onChange={(e) => handleStatusChange(item.feedback.id, e.target.value as feedback_board.FeedbackStatus)}
                          className={`pl-3 pr-8 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-wider border-none outline-none appearance-none cursor-pointer shadow-sm ${STATUS_CONFIG[item.feedback.status].bg} ${STATUS_CONFIG[item.feedback.status].color}`}
                        >
                          {Object.entries(STATUS_CONFIG).map(([key, val]) => (
                            <option key={key} value={key}>
                              {val.label}
                        ...
                        <div className={`absolute right-2 top-1/2 -translate-y-1/2 pointer-events-none ${STATUS_CONFIG[item.feedback.status].color}`}>
                    ...
                    {item.feedback.description && (
                      <p className="text-zinc-500 text-sm leading-relaxed mb-5 line-clamp-2 font-medium">{item.feedback.description}</p>
                    )}
                        ...
                        <div className="flex items-center gap-2">
                          <div className="w-6 h-6 rounded-full bg-zinc-100 border border-zinc-200 overflow-hidden flex items-center justify-center">
                            {item.feedback.author_avatar ? (
                              <img src={item.feedback.author_avatar} alt={item.feedback.author_name || ""} className="w-full h-full object-cover" />
                            ) : (
                              <User size={14} className="text-zinc-400" />
                            )}
                          </div>
                          <span className="text-xs font-bold text-zinc-600">{item.feedback.author_name || "Anonim"}</span>
                        </div>
          ...
          <Drawer.Content className="bg-white flex flex-col rounded-t-[2.5rem] fixed bottom-0 left-0 right-0 max-h-[90dvh] outline-none z-50 max-w-2xl mx-auto border-t border-zinc-200 shadow-2xl">
            <div className="p-8 overflow-y-auto">
              ...
              <div className="flex items-center gap-4 mb-2">
                ...
                <div>
                  <Drawer.Title className="text-2xl font-black tracking-tight text-zinc-900">Paylaş ve Entegre Et</Drawer.Title>
              ...
              <div className="space-y-8 mt-10">
                <div className="space-y-3">
                  ...
                  <div className="flex gap-2">
                    <div className="flex-1 bg-zinc-50 border border-zinc-200 rounded-2xl px-4 py-3 text-sm font-bold text-zinc-600 truncate">{publicUrl}</div>
                    <button
                ...
                <div className="space-y-3">
                  ...
                  <div className="relative">
                    <pre className="bg-zinc-900 text-zinc-300 p-5 rounded-2xl text-xs font-mono overflow-x-auto leading-relaxed border border-zinc-800">{iframeCode}</pre>
                    <button
