    ...
    <div className="min-h-screen text-stone-850 bg-[#fbf9f4] flex flex-col relative overflow-x-hidden">
              ...
              <div className="space-y-4">
                <div className="space-y-3">
                  ...
                  <input
                    ...
                    className="flex-1 px-4 py-2.5 bg-stone-50 border border-stone-200 rounded-xl text-stone-850 text-xs focus:outline-none focus:border-amber-600 focus:ring-1 focus:ring-amber-500/15 transition-all placeholder:text-stone-400"
                  />
                  <button
                    type="submit"
              ...
              <form onSubmit={handleImportGeeklist} className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    placeholder="örn: https://boardgamegeek.com/geeklist/357843 veya 357843"
                    className="w-full px-4 py-3 bg-stone-50 border border-stone-200 rounded-xl text-stone-850 text-sm focus:outline-none focus:border-amber-600 focus:ring-1 focus:ring-amber-500/15 transition-all placeholder:text-stone-400"
                  />
                </div>

                {importingGeeklist && (
                  <div className="flex items-center gap-2.5 p-3.5 bg-amber-50 border border-amber-200 rounded-xl text-amber-700 text-xs">
                    <CircleNotch size={14} className="animate-spin flex-shrink-0" />
                    <span>Geeklist taranıyor ve oyunlar detaylarıyla indiriliyor. Bu işlem 15-30 saniye sürebilir...</span>
                  </div>
                )}
                ...
                <div className="flex items-center justify-end gap-3 pt-3">
                  ...
                  <button
                    type="submit"
                    disabled={importingGeeklist}
                    className="flex items-center justify-center gap-2 bg-amber-700 hover:bg-amber-600 disabled:bg-amber-700/50 text-white font-bold px-6 py-2.5 rounded-xl text-xs transition-all shadow-md"
                  >
                    {importingGeeklist && <CircleNotch size={12} className="animate-spin" />}
                    <span>{importingGeeklist ? "Aktarılıyor..." : "Toplu Aktar"}</span>
                  </button>
                </div>
              </form>
            </div>
      ...
      {isEditModalOpen && selectedGame && (
        ...
        >
          ...
          >
            <div className="relative">
              <h3 className="text-base font-bold text-stone-800 mb-1 truncate">
                &quot;{selectedGame.title}&quot; Durumunu Düzenle
              </h3>
              <p className="text-xs text-stone-500 mb-6">
                Oyunun durum, kondisyon ve notlarını güncelleyin.
              </p>

              <form onSubmit={handleUpdateGame} className="space-y-4">
                <div>
                  <label className="block text-xs font-semibold text-stone-500 mb-2 tracking-wide uppercase">Bulunabilirlik Durumu</label>
                  <select
                    value={editStatus}
                    onChange={(e) => setEditStatus(e.target.value as any)}
                    className="w-full px-4 py-3 bg-stone-50 border border-stone-200 rounded-xl text-stone-850 text-sm focus:outline-none focus:border-amber-600 cursor-pointer"
                  >
                    <option value="available">Mevcut (Kitaplıkta)</option>
                    <option value="borrowed">Ödünç Verildi (Üyede)</option>
                    <option value="maintenance">Bakımda (Hasarlı / Eksik parça inceleniyor)</option>
                  </select>
                </div>

                <div>
                  <label className="block text-xs font-semibold text-stone-500 mb-2 tracking-wide uppercase">Fiziksel Kondisyon</label>
                  <select
                    value={editCondition}
                    onChange={(e) => setEditCondition(e.target.value as any)}
                    className="w-full px-4 py-3 bg-stone-50 border border-stone-200 rounded-xl text-stone-850 text-sm focus:outline-none focus:border-amber-600 cursor-pointer"
                  >
                    <option value="new">Yeni Gibi (Sıfır kondisyon)</option>
                    <option value="good">İyi (Kullanılabilir durumda)</option>
                    <option value="worn">Yıpranmış (Hırpalanmış kutu/parçalar)</option>
                    <option value="damaged">Hasarlı (Oynamayı etkileyen yırtılma/kırılma)</option>
                  </select>
                </div>

                <div>
                  <label className="block text-xs font-semibold text-stone-500 mb-2 tracking-wide uppercase">Özel Notlar</label>
                  <textarea
                    value={editNotes}
                    onChange={(e) => setEditNotes(e.target.value)}
                    placeholder="Kutu durumu, eksik parçalar veya ödünç alan kişi ismi..."
                    rows={3}
                    className="w-full px-3.5 py-2.5 bg-stone-50 border border-stone-200 rounded-xl text-stone-100 text-xs focus:outline-none focus:border-amber-600 resize-none transition-all placeholder:text-stone-400"
                  />
                </div>
                ...
                <div className="flex items-center justify-end gap-3 pt-3">
                  <button
                    type="button"
                    onClick={() => setIsEditModalOpen(false)}
                    className="px-5 py-2.5 bg-stone-100 hover:bg-stone-200/70 border border-[#e8e4d9] rounded-xl text-stone-600 hover:text-stone-850 transition-all text-xs font-semibold"
                  >
                    Vazgeç
                  </button>
                  <button
                    ...
                    className="flex items-center justify-center gap-2 bg-amber-700 hover:bg-amber-600 text-white font-bold px-6 py-2.5 rounded-xl text-xs transition-all shadow-md"
                  >
