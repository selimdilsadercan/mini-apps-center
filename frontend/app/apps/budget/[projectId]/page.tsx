...
const CATEGORIES = [
  ...
  { id: "personal", emoji: "⛺", labelTr: "Kalıcı", labelEn: "Durable" },
...
export default function ProjectDetailsPage() {
  ...
  const fetchProjectDetails = async (silent = false) => {
    try {
      ...
      calculateBalances(res.members, res.expenses, res.shares);
    } catch (error) {
      console.error(error);
          ...
          <>
            ...
            <div className="w-full max-w-md mx-auto">
              ...
              <div className="flex justify-around bg-white border border-gray-200 rounded-2xl py-4 px-2 mb-6 shadow-sm">
                ...
                <div className="flex flex-col items-center justify-center text-center">
                  <span className="text-[10px] text-gray-400 font-semibold mb-1 uppercase tracking-wider">{t.totalSpent}</span>
                  <span className="text-sm font-extrabold text-gray-900">
                    {getCurrencySymbol(project.currency)}{formatVal(totalSpent)}
                  </span>
                </div>
              </div>
            </div>
                          ...
                          {items.map(expense => (
                            ...
                            >
                              ...
                              <div className="flex flex-col items-end justify-center min-h-[44px]">
                                <span className="text-sm font-extrabold text-gray-900">
                                  {getCurrencySymbol(project.currency)}{formatVal(Number(expense.amount))}
                                </span>
                              </div>
                            </div>
                          ))}
