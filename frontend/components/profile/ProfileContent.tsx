...
export function ProfileRow({
  ...
  return (
    <div className="w-full hover:bg-app-surface-muted/30 transition-colors">
      {content}
    </div>
  );
}
...
export function ProfileContent({ embedded = false }: ProfileContentProps) {
  const { user } = useUser();
  ...
  const profileBasePath = hubPath("profile");
  const homePath = hubPath("today");
  ...
  useEffect(() => {
    ...
    const handleBadgeUpdate = (e: CustomEvent<{ hasPending: boolean }>) => {
      setHasFriendsBadge(e.detail.hasPending);
    };

    window.addEventListener("incoming-requests-badge", handleBadgeUpdate as EventListener);
    return () => {
      window.removeEventListener("incoming-requests-badge", handleBadgeUpdate as EventListener);
    };
  }, []);
  ...
  const userEmail =
    (user as { primaryEmailAddress?: { emailAddress?: string }; emailAddresses?: { emailAddress?: string }[] })
    ...
    <div className={`flex flex-col ${embedded ? "flex-1" : "min-h-screen"} bg-app-bg text-app-text pb-8`}>
      ...
      {embedded && (
        <div className="hidden md:block px-4 md:px-8 pt-6 pb-2 max-w-2xl mx-auto w-full">
          <h1 className="text-xl font-black text-app-text tracking-tight">{t("title")}</h1>
        </div>
      )}

      <main className="px-4 md:px-8 pt-4 pb-safe-area-inset-bottom max-w-lg md:max-w-2xl mx-auto w-full space-y-6">
        <div className="rounded-3xl border border-app-border bg-app-surface shadow-sm p-6 flex flex-col items-center text-center relative">
          ...
          <h2 className="text-base font-black text-app-text tracking-tight lowercase">{handleName}</h2>
          {userEmail && (
            <p className="text-[11px] font-bold text-app-muted mt-0.5 lowercase">{userEmail}</p>
          )}
